...
public class Pixel implements Comparable<Pixel> {
    ...
    public int[] properRGB(int[] c) {
        int[] out = {0, 0, 0};
        if (c == null) {
            return out;
        }
        for (int i = 0; i < c.length && i < out.length; i++) {
            if (c[i] < 0) {
                out[i] = 0;
            } else if (c[i] > 255) {
                out[i] = 255;
    ...
    public int[] getComponents() {
        int[] out = {this.r, this.g, this.b};
        return out; // Implement this method
    }
    ...
    public int distance(Pixel px) {
        ...
        int dr = Math.abs(px.r - this.r);
        int dg = Math.abs(px.g - this.g);
        int db = Math.abs(px.b - this.b);

        return dr + dg + db; // Implement this method
    }
    ...
    public String toString() {
        String s = "(" + this.r + ", " + this.g + ", " + this.b + ")";
        return s; // Implement this method
    }
