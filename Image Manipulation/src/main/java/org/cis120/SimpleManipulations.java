...
public class SimpleManipulations {
    ...
    public static PixelPicture border(
        ...
        for (int row = borderWidth; row < h; row++) {
            for (int col = w - borderWidth; col < w; col++) {
                tgt[row][col] = borderColor;
            }
        }

        for (int row = h - borderWidth; row < h; row++) {
    ...
    public static PixelPicture alphaBlend(
        ...
        if (bmp1.length != bmp2.length || bmp1[1].length != bmp2[1].length) {
            return pic;
        }

        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                Pixel p = bmp1[row][col];
                Pixel q = bmp2[row][col];

                int r = weightedAverage(alpha, p.getRed(), q.getRed());
                int g = weightedAverage(alpha, p.getGreen(), q.getGreen());
                int b = weightedAverage(alpha, p.getBlue(), q.getBlue());
