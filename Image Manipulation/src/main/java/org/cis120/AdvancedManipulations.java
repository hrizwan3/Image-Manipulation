...
public class AdvancedManipulations {
    ...
    public static PixelPicture adjustContrast(
        ...
        int avg = (int) Math.round(sum / (w * h * 3.0));
    ...
    public static PixelPicture reducePalette(PixelPicture pic, int numColors) {
        ...
        Pixel[] hiFreqAll = m.getSortedPixels();
        Pixel[] palette = new Pixel[numColors];
        System.arraycopy(hiFreqAll, 0, palette, 0, numColors);

        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                Pixel p = bmp[row][col];
                int minDist = Integer.MAX_VALUE;
                Pixel minDistPixel = new Pixel(0, 0, 0);
                for (int i = 0; i < numColors; i++) {
                    if (p.distance(palette[i]) < minDist) {
                        minDist = p.distance(palette[i]);
                        minDistPixel = palette[i];
                    }
                }
                bmp[row][col] = minDistPixel;
            }
        }
        return new PixelPicture(bmp);
    }
    ...
    public static PixelPicture blur(PixelPicture pic, int radius) {
        ...
        // edge cases: at any of the corners or row (or col) + rad > w (or h)
        // or row (or col) - rad < 0

        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                out[row][col] =
                        avgColorHelper(bmp, row - radius, row + radius,
                                col - radius, col + radius, w, h);
            }
        }
        return new PixelPicture(out);
    }

    // NOTE: You may want to add a static helper function here to
    // help find the average color around the pixel you are blurring.
    public static Pixel avgColorHelper(Pixel[][] arr, int r1, int r2,
                                       int c1, int c2, int w, int h) {
        int countR = 0;
        int countG = 0;
        int countB = 0;
        int div = 0;
        ...
        int avgR = (int) (Math.round(countR / (double) (div)));
        int avgG = (int) (Math.round(countG / (double) (div)));
        int avgB = (int) (Math.round(countB / (double) (div)));
